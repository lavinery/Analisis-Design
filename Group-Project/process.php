...
if (isset($_POST['update-article'])) {
    ...
    if ($_FILES['article-image']['name'] != "") {
        ...
        $uploadname     = $_FILES['article-image']['name'];
        $uploadtmp      = $_FILES['article-image']['tmp_name'];
        $nameuploaded   = $username . " " . $uploadname;
        $uptquery = mysqli_query($conn, "SELECT * FROM tbl_konten WHERE id_konten = '$article_id'");
        $row    = mysqli_fetch_array($uptquery);
        if (file_exists($uploaddir . $row['gambar'])) {
        ...
        $sql = "UPDATE tbl_konten SET judul_konten = '$article_title', isi_konten = '$article_content', gambar = '$nameuploaded' WHERE id_konten = '$article_id'";
    } else {
        $sql = "UPDATE tbl_konten SET judul_konten = '$article_title', isi_konten = '$article_content' WHERE id_konten = '$article_id'";
    }

    $query = mysqli_query($conn, $sql);
    if ($query) {
        echo "<script>alert('Telah berhasil merubah data'); window.location.href = 'dashboard.php'</script>";
    }
}
